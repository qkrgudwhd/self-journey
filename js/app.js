...
const S = {
  ...
  mbtiMode: 'short', mbtiType: '', mbtiPick: {},   // 성향: quick(직접선택)/short(16문항)/full(92문항)
  result: {},
};
...
// ---------- 화면 전환 ----------
const STEPS = ['basic', 'shape', 'color', 'face', 'palm', 'mbti', 'summary'];
const STEP_LABELS = { basic:'1/7 기본 정보', shape:'2/7 도형 선택', color:'3/7 나의 색',
                      face:'4/7 관상 사진', palm:'5/7 손금 사진', mbti:'6/7 성향 검사', summary:'7/7 결과' };
...
function go(id) {
  ...
  if (id === 'mbti') mbtiChoose();
  if (id === 'face') initFace();
  if (id === 'palm') initPalm();
  if (id === 'taekil') initTaekil();
  if (id === 'dream') initDream();
}
...
// ---------- 생년월일·시각 드롭다운 (달력 대체) ----------
function fillSel(id, from, to, fmt, sel, leadEmpty) {
  const el = document.getElementById(id); if (!el) return;
  let h = leadEmpty ? `<option value="">모름</option>` : '';
  for (let v = from; v <= to; v++) h += `<option value="${v}">${fmt(v)}</option>`;
  el.innerHTML = h;
  if (sel != null) el.value = String(sel);
}
function daysInMonthFor(cal, y, m) {
  if (cal === 'lunar') return 30;                 // 음력은 29~30일 → 30까지 허용(변환 시 검증)
  return new Date(y, m, 0).getDate();             // 양력 실제 일수(윤년 반영)
}
function refreshDays(prefix) {
  const cal = (document.getElementById(prefix + 'cal') || {}).value || 'solar';
  const y = +document.getElementById(prefix + 'year').value || 2000;
  const m = +document.getElementById(prefix + 'month').value || 1;
  const dEl = document.getElementById(prefix + 'day');
  const cur = +dEl.value || 1;
  const max = daysInMonthFor(cal, y, m);
  fillSel(prefix + 'day', 1, max, v => v + '일', Math.min(cur, max));
}
function initDateSelects(prefix, def, unknownTime) {
  fillSel(prefix + 'year', 1901, 2049, v => v + '년', def.year);
  fillSel(prefix + 'month', 1, 12, v => v + '월', def.month);
  refreshDays(prefix);
  document.getElementById(prefix + 'day').value = String(def.day);
  fillSel(prefix + 'hour', 0, 23, v => v + '시', unknownTime ? null : def.hour, unknownTime);
  fillSel(prefix + 'min', 0, 59, v => v + '분', def.min);
  ['year', 'month', 'cal'].forEach(k => {
    const el = document.getElementById(prefix + k);
    if (el) el.addEventListener('change', () => refreshDays(prefix));
  });
}
initDateSelects('in-', { year: 1990, month: 1, day: 1, hour: 12, min: 0 }, false);
initDateSelects('gh-', { year: 1990, month: 1, day: 1, hour: 12, min: 0 }, true);
...
document.getElementById('gh-cal').onchange = e => {
  document.getElementById('gh-leap-wrap').style.display = e.target.value === 'lunar' ? 'flex' : 'none';
};
...
function submitBasic() {
  ...
  const d = +document.getElementById('in-day').value;
  S.date = `${y}-${pad2(m)}-${pad2(d)}`;
  S.cal = document.getElementById('in-cal').value;
  S.leap = document.getElementById('in-leap').checked;
  S.noHour = document.getElementById('in-nohour').checked;
  const hh = +document.getElementById('in-hour').value, mi = +document.getElementById('in-min').value;
  S.time = `${pad2(hh)}:${pad2(mi)}`;
  ...
  if (!y || y < 1901 || y > 2049) { err.textContent = '1901~2049년생만 지원합니다.'; return; }
  err.textContent = '';
  go('shape');
}
...
// ---------- STEP 6 성향(MBTI) ----------
let mqIdx = 0;
let activeQ = QUESTIONS;        // 진행 중인 문항 세트 (short=16 / full=92)
const AXIS_LABEL = { EI:'에너지의 방향', SN:'정보를 받아들이는 방식', TF:'판단의 기준', JP:'삶을 대하는 태도' };
const AXIS_PAIRS = [
  { key:'EI', theme:'에너지의 방향', a:['E','외향 · 밖에서 기운을 얻는다'], b:['I','내향 · 안에서 기운을 채운다'] },
  { key:'SN', theme:'정보를 받아들이는 방식', a:['S','감각 · 사실과 경험을 믿는다'], b:['N','직관 · 의미와 가능성을 본다'] },
  { key:'TF', theme:'판단의 기준', a:['T','사고 · 논리로 판단한다'], b:['F','감정 · 관계로 판단한다'] },
  { key:'JP', theme:'삶을 대하는 태도', a:['J','판단 · 계획하고 정돈한다'], b:['P','인식 · 유연하게 열어둔다'] },
];

// (1) 방법 선택 화면
function mbtiChoose() {
  document.getElementById('mbti-choose').style.display = 'block';
  document.getElementById('mbti-quick').style.display = 'none';
...
function mbtiStart(mode) {
  S.mbtiMode = mode;
  document.getElementById('mbti-choose').style.display = 'none';
  if (mode === 'quick') {
    document.getElementById('mbti-quick').style.display = 'block';
    document.getElementById('mbti-quiz').style.display = 'none';
    renderQuickAxes();
  } else {
    activeQ = mode === 'full' ? QUESTIONS : QUESTIONS_SHORT;
    S.mbtiAnswers = [];
    mqIdx = 0;
    document.getElementById('mbti-quick').style.display = 'none';
    document.getElementById('mbti-quiz').style.display = 'block';
    document.getElementById('mq-kicker').textContent = mode === 'full' ? '정밀 검사 · 92문항' : '간단 검사 · 16문항';
    document.getElementById('mq-title').textContent = mode === 'full' ? '성향 검사 (정밀)' : '성향 검사 (간단)';
    renderMbtiQ();
  }
}

// (2-a) 빠른 4택
function renderQuickAxes() {
  const box = document.getElementById('quick-axes');
  box.innerHTML = '';
  AXIS_PAIRS.forEach(ax => {
    const row = document.createElement('div');
    row.className = 'qa-row';
    row.innerHTML = `<div class="qa-theme">${ax.theme}</div>
      <div class="qa-pair">
        <button class="qa-btn${S.mbtiPick[ax.key]===ax.a[0]?' on':''}" data-k="${ax.key}" data-v="${ax.a[0]}"><b>${ax.a[0]}</b><small>${ax.a[1]}</small></button>
        <button class="qa-btn${S.mbtiPick[ax.key]===ax.b[0]?' on':''}" data-k="${ax.key}" data-v="${ax.b[0]}"><b>${ax.b[0]}</b><small>${ax.b[1]}</small></button>
      </div>`;
    box.appendChild(row);
  });
  box.querySelectorAll('.qa-btn').forEach(btn => {
    btn.onclick = () => {
      ...
      S.mbtiPick[k] = btn.dataset.v;
      box.querySelectorAll(`.qa-btn[data-k="${k}"]`).forEach(x => x.classList.remove('on'));
      btn.classList.add('on');
      const done = AXIS_PAIRS.every(a => S.mbtiPick[a.key]);
      const db = document.getElementById('quick-done');
      db.disabled = !done; db.style.opacity = done ? '1' : '.55';
    };
  });
  const db = document.getElementById('quick-done');
  const done = AXIS_PAIRS.every(a => S.mbtiPick[a.key]);
...
function quickDone() {
  S.mbtiType = AXIS_PAIRS.map(a => S.mbtiPick[a.key]).join('');
  S.mbtiMode = 'quick';
  startAnalysis();
}
...
function renderMbtiQ() {
  const q = activeQ[mqIdx];
  document.getElementById('mq-no').textContent = `${mqIdx + 1} / ${activeQ.length}`;
  document.getElementById('mq-axis').textContent = AXIS_LABEL[q.a];
  document.getElementById('mq-bar').style.width = (mqIdx / activeQ.length * 100) + '%';
  ...
  SCALE_LABELS.forEach((lab, i) => {
    ...
    b.onclick = () => {
      ...
      b.classList.add('sel');
      setTimeout(() => {
        if (mqIdx + 1 < activeQ.length) { mqIdx++; renderMbtiQ(); }
        else startAnalysis();
      }, 130);
    };
    box.appendChild(b);
...
function buildFaceResult() {
  ...
  const res = FaceRead.interpret(FACE.front.metrics, FACE.left && FACE.left.metrics, FACE.right && FACE.right.metrics, S.gender);
  const thumbs = [];
...
function finishFace() { buildFaceResult(); go('palm'); }
function skipFace() { S.result.face = null; S.result.faceThumbs = null; go('palm'); }

// ═══════════════ 손금(수상) — 사진 검출 & 계측 ═══════════════
const PALM = { right: null, left: null };
let palmPickHand = null, palmEngineReady = false;

function initPalm() {
  refreshPalmBtn();
  const el = document.getElementById('palm-engine');
  if (location.protocol === 'file:') {
    el.innerHTML = `<span style="color:#e88aa6">⚠ 손금 기능도 <b>웹서버에서만</b> 작동합니다(파일 직접 열기 file:// 에서는 AI 모델을 못 읽습니다).<br/>
      · <b>온라인 주소</b>(넷리파이·GitHub)에서는 정상 작동합니다.<br/>
      · 내 컴퓨터에서 시험하려면 <b>「관상_테스트_열기.bat」</b>을 더블클릭해 여세요.<br/>
      (손금 없이 넘어가려면 아래 <b>'사진 없이 진행'</b>을 눌러 주세요.)</span>`;
    return;
  }
  if (palmEngineReady) { el.textContent = '✓ 손 인식 엔진 준비됨 — 손바닥 사진을 올려 주세요.'; return; }
  el.textContent = '손 인식 엔진을 준비하는 중… (최초 1회, 잠시 걸립니다)';
  PalmReader.load(msg => { el.textContent = msg; })
    .then(() => { palmEngineReady = true; el.textContent = '✓ 손 인식 엔진 준비 완료 — 손바닥 사진을 올려 주세요.'; })
    .catch(err => {
      const hint = location.protocol === 'file:'
        ? " (파일 직접 열기에서는 작동하지 않습니다 — 「관상_테스트_열기.bat」으로 여세요)"
        : " 브라우저를 새로고침하거나, '사진 없이 진행'을 눌러 주세요.";
...
function pickPalm(hand) {
  if (location.protocol === 'file:') { initPalm(); return; }
  palmPickHand = hand;
  document.getElementById('palm-file').click();
}

function onPalmPicked(input) {
  const file = input.files[0];
  input.value = '';
  if (!file || !palmPickHand) return;
  const hand = palmPickHand;
  const stat = document.getElementById('pstat-' + hand);
  const slot = document.getElementById('pslot-' + hand);
  const url = URL.createObjectURL(file);
  const img = new Image();
  img.onload = () => {
    slot.classList.add('filled');
    stat.className = 'fs-stat fs-busy'; stat.textContent = '손을 찾는 중…';
    PalmReader.load().then(() => {
      let det = null;
      try { det = PalmReader.detect(img); } catch (e) { det = null; }
      const cv = document.getElementById('pcv-' + hand);
      if (!det) {
        PalmReader.drawOverlay(cv, img, null, null);
        URL.revokeObjectURL(url);
        PALM[hand] = null;
        stat.className = 'fs-stat fs-warn';
        stat.textContent = '손을 찾지 못했어요. 손바닥이 크고 밝게 나온 사진으로 다시 시도해 주세요.';
        refreshPalmBtn();
        return;
      }
      let lines = null;
      try { lines = PalmReader.detectLines(img, det.lm); } catch (e) { lines = null; }
      PalmReader.drawOverlay(cv, img, det.lm, lines);
      URL.revokeObjectURL(url);
      const measured = PalmRead.markMeasured({ lm: det.lm, W: img.naturalWidth, H: img.naturalHeight });
      measured.handed = det.handed;
      measured.lines = lines ? lines.clarity : null;
      // 썸네일(오버레이 포함)
      const scale = Math.min(1, 360 / cv.width);
      const tcv = document.createElement('canvas');
      tcv.width = Math.round(cv.width * scale); tcv.height = Math.round(cv.height * scale);
      tcv.getContext('2d').drawImage(cv, 0, 0, tcv.width, tcv.height);
      PALM[hand] = { data: measured, thumb: tcv.toDataURL('image/jpeg', 0.85) };
      stat.className = 'fs-stat fs-ok';
      stat.textContent = hand === 'right' ? '✓ 주 손 검출 완료 (21점)' : '✓ 반대 손 검출 완료';
      refreshPalmBtn();
    }).catch(err => {
      URL.revokeObjectURL(url);
      stat.className = 'fs-stat fs-warn';
...
function refreshPalmBtn() {
  const btn = document.getElementById('btn-palm');
  const has = PALM.right || PALM.left;
  btn.textContent = has ? '손금 넣고 다음 — 성향 검사 ✦' : '손바닥 사진을 올리면 손금 분석이 켜집니다';
  btn.disabled = !has;
  btn.style.opacity = has ? '1' : '.55';
}

function buildPalmResult() {
  const main = PALM.right || PALM.left;
  if (!main) { S.result.palm = null; S.result.palmThumbs = null; return; }
  const res = PalmRead.interpret(PALM.right && PALM.right.data, PALM.left && PALM.left.data, S.gender);
  const thumbs = [];
  if (PALM.right) thumbs.push({ url: PALM.right.thumb, label: '주로 쓰는 손' });
  if (PALM.left) thumbs.push({ url: PALM.left.thumb, label: '반대 손' });
  S.result.palm = res;
  S.result.palmThumbs = thumbs;
}

function finishPalm() { buildPalmResult(); go('mbti'); }
function skipPalm() { S.result.palm = null; S.result.palmThumbs = null; go('mbti'); }

// ---------- 분석 ----------
const LOAD_MSGS = ['절기 시각을 대조하는 중', '이름의 오행을 읽는 중', '색의 결을 고르는 중',
                   '92개의 답을 저울에 다는 중', '네 개의 기둥을 세우는 중',
                   '얼굴의 삼정을 재는 중', '손금의 세 갈래 선을 읽는 중'];
...
function runEngines() {
  ...
  // 성향: 직접 선택(quick) / 간단 16문항(short) / 정밀 92문항(full)
  if (S.mbtiMode === 'quick') S.result.mbti = mbtiFromType(S.mbtiType);
  else S.result.mbti = scoreAssessment(S.mbtiAnswers, S.mbtiMode === 'full' ? QUESTIONS : QUESTIONS_SHORT);
...
const HANJA_KO = { 命:'명', 名:'명', 緣:'연', 運:'운', 像:'상', 相:'상', 手:'수', 格:'격', 跡:'적' };
...
function renderSummary() {
  ...
  // 손금 카드 (사진을 넣은 경우에만)
  if (R.palm && R.palm.ok) {
    html += card('手', '손금 — 손에 그려진 나', R.palm.hand.info.key, PalmRead.summaryHtml(R.palm));
  }
...
function saveProfile() {
  ...
  profiles[key] = {
    ...
    data: { sur:S.sur, giv:S.giv, gender:S.gender, blood:S.blood,
            ...
            mbtiMode:S.mbtiMode, mbtiType:S.mbtiType },
  };
...
function loadProfile(key) {
  ...
  // 구버전 프로필 보정: 성향 모드가 없으면 응답 길이로 추정
  if (!p.data.mbtiMode) S.mbtiMode = (p.data.mbtiAnswers && p.data.mbtiAnswers.length === QUESTIONS.length) ? 'full' : 'short';
  if (!p.data.mbtiType) S.mbtiType = '';
  S.result.face = null; S.result.faceThumbs = null;   // 저장 프로필엔 사진이 없음(관상·손금 제외)
  S.result.palm = null; S.result.palmThumbs = null;
  runEngines();
  renderSummary();
  go('summary');
}
...
function renderReport() {
  ...
  // ─── 제6장 手 (손금) ───
  if (R.palm && R.palm.ok) {
    const palmInner = PalmRead.reportHtml(R.palm, R.palmThumbs);
    html += chapter('手', '제6장 · 손에 그려진 나', '손금 — 손 유형·손가락·3대선', palmInner);
  }
  ...
  i3 += sec(`${mb.type} — ${mb.typeName}`,
    ...
    (mb.selfReported ? `<p class="sub">본인이 직접 선택하신 유형입니다. 축별 강도는 검사가 아닌 표시용 기본값이며, 정확한 강도를 보시려면 간단·정밀 검사를 이용하세요.</p>` : ''));
  i3 += sec('삶의 세 장면',
  ...
  html += chapter('格', '제7장 · 성향과 강점의 조화', '데이터가 말하는 나의 페르소나', i3);
  ...
  html += chapter('跡', '제8장 · 나를 쓰다', '시간의 궤적과 남겨질 유산', i4);
...
function runGunghap() {
  ...
  const y = +document.getElementById('gh-year').value;
  const m = +document.getElementById('gh-month').value;
  const d = +document.getElementById('gh-day').value;
  const leap = document.getElementById('gh-leap').checked;
  const hRaw = document.getElementById('gh-hour').value;   // ''이면 시각 모름
  const hh = hRaw === '' ? null : +hRaw;
  const mi = hRaw === '' ? 0 : +document.getElementById('gh-min').value;

  if (!y) { err.textContent = '상대의 생년월일을 선택해 주세요.'; return; }
  ...
  const other = computeSaju({ year: y, month: m, day: d, hour: hh, minute: mi,
                              calendar: cal, leapMonth: leap, gender: ghGender });
...
function openFengshui() {
  const R = S.result;
  if (!R.saju || R.saju.error) { alert('먼저 기본 정보로 사주가 계산되어야 합니다.'); return; }
  const yr = (R.saju.meta && R.saju.meta.sajuYear) || R.saju.input.solarYear;
  const f = fengshuiReading(yr, S.gender, R.saju.elements);
  const rows = f.directions.map(d => {
    const color = d.grade.indexOf('길') >= 0 ? 'var(--leaf)' : (d.grade.indexOf('흉') >= 0 ? '#e88aa6' : 'var(--dim)');
    return `<tr><td>${d.dir}쪽</td><td class="v">${d.yeonyun}</td><td style="color:${color};font-weight:700">${d.grade}</td><td class="j">${d.use}</td></tr>`;
  }).join('');
  document.getElementById('fengshui-result').innerHTML = `
    <div class="rpt-cover" style="padding:24px 10px 6px">
      <div class="kicker">生活風水</div>
      <div class="big" style="font-size:24px">${S.sur}${S.giv} 님의 방위</div>
      <p class="hint">${f.starName} · 본명궁 ${f.guaName} · ${f.house}</p>
    </div>
    <div class="rpt-sec"><h4>본명궁과 사택 — ${f.house}</h4><p>${f.houseNote}</p>
      <p class="sub">태어난 해로 세운 구성(九星) 본명성입니다. 같은 사택끼리의 방위가 나에게 길합니다.</p></div>
    <div class="rpt-sec"><h4>여덟 방위 길흉 (팔택 유년)</h4>
      <div class="mtab-wrap"><table class="mtab"><thead><tr><th>방위</th><th>유년</th><th>길흉</th><th>쓰임</th></tr></thead><tbody>${rows}</tbody></table></div></div>
    <div class="rpt-sec rel-sec"><h4>배치 권고</h4>${f.advice.map(a => `<p>· ${a}</p>`).join('')}
      ${f.elemAdvice ? `<p style="margin-top:8px">🎨 ${f.elemAdvice}</p>` : ''}
      <p class="sub">팔택명경(八宅明鏡)의 유년 배치를 따른 전통 참고입니다. 방위는 집의 중심에서 나침반으로 봅니다.</p></div>`;
  go('fengshui');
}

// ---- 이사택일 ----
function openTaekil() { go('taekil'); }
function initTaekil() {
  const msel = document.getElementById('tk-month');
  ...
  const yEl = document.getElementById('tk-year');
  const now = new Date();
  if (yEl && !yEl.value) yEl.value = now.getFullYear();
  if (msel && !msel.value) msel.value = Math.min(12, now.getMonth() + 2);
}
function runTaekil() {
  const err = document.getElementById('tk-err');
  const y = +document.getElementById('tk-year').value, m = +document.getElementById('tk-month').value;
  if (!y || y < 1901 || y > 2049) { err.textContent = '1901~2049년 사이 연도를 입력해 주세요.'; return; }
  err.textContent = '';
  const R = S.result;
  let myYB = null, myZodiac = '';
  if (R.saju && !R.saju.error) {
    const yj = R.saju.pillars.find(p => p.name === '년주');
    if (yj) myYB = yj.branch;
    myZodiac = R.saju.zodiac;
  }
  const res = taekilScan(y, m, myYB);
  if (res.error) { err.textContent = res.error; return; }
  const picks = res.picks.length
    ? res.picks.map(p => `<span class="tag gold">${res.month}월 ${p.day}일(${p.weekday}) ${p.mark}</span>`).join(' ')
    : '<span class="sub">이 달엔 손없는 날과 황도가 겹치는 날이 적습니다. 아래 표의 ★를 참고하세요.</span>';
  const rows = res.days.map(d => {
    const tags = [];
    if (d.sonEobs) tags.push('손없는날'); else if (d.sonDir) tags.push('손:' + d.sonDir + '쪽');
    tags.push(d.hwangdo ? '황도(' + d.shin + ')' : d.shin);
    if (d.chung) tags.push('띠충');
    const color = d.grade === '최길' ? 'var(--gold)' : d.grade === '피함' ? '#e88aa6' : d.grade === '길' ? 'var(--leaf)' : 'var(--dim)';
    return `<tr><td>${d.day}(${d.weekday})</td><td class="j">음${d.lunar}</td><td>${d.ilji}</td><td style="color:${color};font-weight:700">${d.mark} ${d.grade}</td><td class="j">${tags.join(' · ')}</td></tr>`;
  }).join('');
  document.getElementById('taekil-result').innerHTML = `
    <div class="rpt-sec"><h4>${y}년 이사 방위 (피할 방위)</h4><p>${res.yearInfo.note}</p>
      ${myZodiac ? `<p class="sub">본인 ${myZodiac} — 그날 일진이 본인 띠와 충(沖)인 날(×)은 길일에서 제외했습니다.</p>` : ''}</div>
    <div class="rpt-sec rel-sec"><h4>추천 이사 길일 · ${y}년 ${m}월</h4><p>${picks}</p>
      <p class="sub">★★★ 최길(손없는날+황도) · ★★ 길 · × 피함(띠충)</p></div>
    <div class="rpt-sec"><h4>${m}월 전체 판정표</h4>
      <div class="mtab-wrap"><table class="mtab"><thead><tr><th>날짜</th><th>음력</th><th>일진</th><th>판정</th><th>근거</th></tr></thead><tbody>${rows}</tbody></table></div>
      <p class="sub">손없는 날(음력 9·10·19·20·29·30)과 황도길일이 겹치고 본인 띠와 충하지 않는 날을 길일로 봅니다. 전통 택일법 참고 자료입니다.</p></div>`;
  document.getElementById('taekil-result').scrollIntoView({ behavior: 'smooth' });
}

// ---- 꿈해몽 ----
function initDream() {
  const tags = document.getElementById('dream-tags');
  if (tags && !tags.dataset.done && typeof dreamPopular === 'function') {
    tags.innerHTML = dreamPopular().map(w =>
      `<button class="chip" onclick="document.getElementById('dream-q').value='${w}';runDream()">${w}</button>`).join(' ');
    tags.dataset.done = '1';
  }
}
function runDream() {
  const q = document.getElementById('dream-q').value;
  const box = document.getElementById('dream-result');
  if (typeof searchDreams !== 'function' || typeof DREAM_DB === 'undefined') {
    box.innerHTML = '<div class="rpt-sec"><p class="sub">꿈해몽 사전을 아직 불러오지 못했습니다. 새로고침 후 다시 시도해 주세요.</p></div>';
    return;
  }
  if (!q.trim()) { box.innerHTML = ''; return; }
  const res = searchDreams(q);
  if (!res.length) {
    box.innerHTML = `<div class="rpt-sec"><p>'${q}'에 대한 풀이를 찾지 못했습니다. 더 단순한 단어(예: 물, 뱀, 돈, 불)로 검색해 보세요.</p></div>`;
    return;
  }
  const LUCK = { '길몽':'var(--leaf)', '흉몽':'#e88aa6', '중립':'var(--dim)' };
  box.innerHTML = res.map(e => {
    const c = LUCK[e.luck] || 'var(--line)';
    return `<div class="rpt-sec"><h4>${e.kw[0]} <span class="tag" style="border-color:${c};color:${c}">${e.luck}</span> <span class="sub" style="font-weight:400">${e.cat}</span></h4>
      <p><b>${e.short}</b></p><p>${e.text}</p></div>`;
  }).join('');
