...
function scoreAssessment(answers, questions) {
  const Q = questions || QUESTIONS;                     // 문항 세트 선택(기본=92문항)
  if (!Array.isArray(answers) || answers.length !== Q.length)
    return { error: `응답 ${Q.length}개가 필요합니다 (현재 ${answers ? answers.length : 0}개).` };
...
/**
 * 유형을 직접 아는 경우 — 네 글자로 결과 구성 (자가 선택)
 * type: 'ENFJ' 같은 4글자
 */
function mbtiFromType(type) {
  type = (type || '').toUpperCase();
  if (!/^[EI][SN][TF][JP]$/.test(type)) return { error: '유형 형식이 올바르지 않습니다 (예: ENFJ).' };
  const axesOut = AXES.map((ax, i) => {
    const letter = type[i], isFirst = letter === ax.first;
    const firstPct = isFirst ? 70 : 30, secondPct = 100 - firstPct;
    return {
      axis: ax.key, theme: ax.theme,
      first: ax.first, firstName: ax.firstName, firstPct,
      second: ax.second, secondName: ax.secondName, secondPct,
      letter, letterName: isFirst ? ax.firstName : ax.secondName,
      pct: 70, strength: '직접 선택', selfReported: true,
    };
  });
  return { type, typeName: TYPE_INFO[type].name, brief: TYPE_INFO[type].brief, axes: axesOut, selfReported: true };
}

if (typeof module !== 'undefined') module.exports = { scoreAssessment, mbtiFromType, AXES, TYPE_INFO };
